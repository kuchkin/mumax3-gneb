...
addzhanglitorque4D(float* __restrict__ n, float* __restrict__ l,
    ...
    float3 torque = (-1.0f/(1.0f + alpha*alpha)) * (-(1.0f+xi*alpha)*(hspin) +(  xi-alpha) * cross(m, hspin)           );
                        //* cross(m, cross(m, hspin))
    ...
    float pref = 0.0;
    if(J.x != 0.0f) pref += (b/cx)*deltax(n)*J.x;
    if(J.y != 0.0f) pref += (b/cy)*deltay(n)*J.y;
    if(J.z != 0.0f) pref += (b/cz)*deltaz(n)*J.z;

    l[i]  += ((1.0f+xi*alpha)/(1.0f + alpha*alpha))*pref;
}
