...
gneb_adddmi(float* __restrict__ Hx, float* __restrict__ Hy, float* __restrict__ Hz,
    ...
    // x derivatives (along length)
    {
        ...
        float pref = (abs(A1)>0)? 1.0/A1: 0.0;                 // inter-region Dex
        if (!is0(m1) || !OpenBC){                          // do nothing at an open boundary
            if (is0(m1)) {                                 // neighbor missing
                m1.x = m0.x - (-cx * (0.5f*D1*pref) * m0.z); // extrapolate missing m from Neumann BC's
                m1.y = m0.y;
                m1.z = m0.z + (-cx * (0.5f*D1*pref) * m0.x);
            }
...

    {
        ...
        float pref = (abs(A2)>0)? 1.0/A2: 0.0; 
        if (!is0(m2) || !OpenBC){
            if (is0(m2)) {
                m2.x = m0.x - (cx * (0.5f*D2*pref) * m0.z);
                m2.y = m0.y;
                m2.z = m0.z + (cx * (0.5f*D2*pref) * m0.x);
            }
    ...
    // y derivatives (along height)
    {
        ...
        float D1 = dLUT2d[symidx(r0, r1)];
        float pref = (abs(A1)>0)? 1.0/A1: 0.0; 
        if (!is0(m1) || !OpenBC){
            if (is0(m1)) {
                m1.x = m0.x;
                m1.y = m0.y - (-cy * (0.5f*D1*pref) * m0.z);
                m1.z = m0.z + (-cy * (0.5f*D1*pref) * m0.y);
            }
...

    {
        ...
        if (!is0(m2) || !OpenBC){
            if (is0(m2)) {
                m2.x = m0.x;
                m2.y = m0.y - (cy * (0.5f*D2*pref) * m0.z);
                m2.z = m0.z + (cy * (0.5f*D2*pref) * m0.y);
            }
